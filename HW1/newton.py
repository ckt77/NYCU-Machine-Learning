...
def plot_regression_comparison(x, y, coefficients_lse, coefficients_newton, degree, lambd):
    plt.figure(figsize=(8, 10))

    x_line = np.linspace(min(x) - 1, max(x) + 1, 100)
    y_line_lse = matrix_multiplication(polynomial_basis(x_line, degree), coefficients_lse).flatten()
    y_line_newton = matrix_multiplication(polynomial_basis(x_line, degree), coefficients_newton).flatten()
    y_min = min(min(y_line_lse), min(y_line_newton), min(y)) - 10
    y_max = max(max(y_line_lse), max(y_line_newton), max(y)) + 10

    # LSE
    plt.subplot(2, 1, 1)
    plt.scatter(x, y, label='Data points', color='blue')
    plt.plot(x_line, y_line_lse, color='red', label='LSE Polynomial Curve')
    # plt.xlabel('x')
    # plt.ylabel('y')
    plt.legend()
    plt.title(f'LSE Polynomial Regression (Degree {degree}) with λ={lambd}')
    plt.ylim(y_min, y_max)

    # Newton's Method
    plt.subplot(2, 1, 2)
    plt.scatter(x, y, label='Data points', color='blue')
    plt.plot(x_line, y_line_newton, color='green', label="Newton's Method Polynomial Curve")
    # plt.xlabel('x')
    # plt.ylabel('y')
    plt.legend()
    plt.title(f"Newton's Method Polynomial Regression (Degree {degree})")
    plt.ylim(y_min, y_max)
...
def linear_regression_via_newton(x, y, degree):
    A = polynomial_basis(x, degree)
    coefficients = newton_method(A, y, degree)
    coefficients_reshape = coefficients.reshape(-1, 1)  # (degree, ) -> (degree, 1)
...
coefficients_lse = rLSE(x, y, degree, lambd)
print()
coefficients_newton = linear_regression_via_newton(x, y, degree)

plot_regression_comparison(x, y, coefficients_lse, coefficients_newton, degree, lambd)
